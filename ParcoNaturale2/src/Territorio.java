...
public class Territorio {
	private static final int NUM_INIZIALE_ANIMALI = 1000;
	...
	public Map<Integer, Set<Animale>> anno2animali() {
		Map<Integer, Set<Animale>> anno2erbivori = new HashMap<>();
		for(Animale a : this.getAnimali()) {
			Set<Animale> insiemeErbivori = anno2erbivori.get(a.getAnni());
			if(insiemeErbivori == null) {
				insiemeErbivori = new HashSet<>();
				anno2erbivori.put(a.getAnni(), insiemeErbivori);
			}
			insiemeErbivori.add(a);
		}
		return anno2erbivori;
	}
