...
public class Battlefield {
	...
	public Map<Class, Set<Robot>> raggruppaRobotPerTipo() {
		// (vedi DOMANDA 3)
		Map<Class, Set<Robot>> raggruppatore = new HashMap<>();
		Set<Robot> insieme;
		for(Robot r : this.posizione2robot.values()) {
			insieme = raggruppatore.get(r.getClass());
			if(insieme == null) {
				insieme = new HashSet<>();
				raggruppatore.put(r.getClass(), insieme);
			}
			insieme.add(r);
		}
		return raggruppatore;	
	}
	
	public List<Robot> getRobotOrdinatiPerPosizione() {
		// (vedi DOMANDA 4)
		List<Robot> lista = new ArrayList<>(this.getAllRobots());
	...
	public SortedSet<Robot> getRobotOrdinatiPerLongevita() {
		// (vedi DOMANDA 6)
		SortedSet<Robot> insieme = new TreeSet<>(this.getAllRobots());
		return insieme;
	}
