 ...
 * <B>(VEDI DOMANDA 2)</B>
 */
public class Mezzo implements Comparable<Mezzo>{
	...
	private int numeroRifiutiRaccolti;
	...
	public void passo() {
		...
		case RIENTRO: 
			...
			break;
		case SMALTIMENTO:
			this.smaltiti += this.numeroRifiutiRaccolti;
			this.numeroRifiutiRaccolti = 0;
	...
	private void eseguiRaccolta() {
		...
		/*if (this.getPoliticaBrown()!=null) {
			nuova = this.getPoliticaBrown().decidiDirezione(corrente);
		}
		else if (this.getPoliticaChase()!=null) {
			nuova = this.getPoliticaChase().decidiDirezione(corrente);
		}*/
	...
	@Override
	public int compareTo(Mezzo that) {
		int a = that.smaltiti - this.smaltiti;
		if(a == 0)
			return 1;
		return a;
		/* VARIANTE SOLO PER FAR VEDERE COME FUNZIONA
		int cmp = this.getCarico().size() - that.getCarico().size();
		if(cmp == 0)
			cmp = this.getPolitica().getId() - that.getPolitica().getId();
		if(cmp == 0)
			cmp = this.getClass().getName().compareTo(that.getClass().getName());
		return cmp;*/
	}
	...
	public boolean equals(Object o) {
		Mezzo that = (Mezzo) o;
		return this.getPolitica().getId() == that.getPolitica().getId() && this.getPolitica().getClass().equals(that.getPolitica().getClass());
	}
